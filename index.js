...
let liveReports = [];
let successPush = 0;
let currentStatus = "Nexus Pro v3 Initializing...";
...
async function runNexusUltra() {
    let browser;
    try {
        currentStatus = "Launching Engine...";
        io.emit('updateStats', { successPush, liveReports, currentStatus });

        browser = await chromium.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
        const context = await browser.newContext({ userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/123.0.0.0 Safari/537.36' });
        ...
        currentStatus = "Injecting Traffic...";
        io.emit('updateStats', { successPush, liveReports, currentStatus });

        await page.goto(targetLink, { waitUntil: 'networkidle', timeout: 60000 });
        
        // AI Scrolling - মেমরি সেভ করার জন্য ছোট লুপ
        ...
        if(liveReports.length > 30) liveReports.pop();
